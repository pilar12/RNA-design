...
class RNADesignFormer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.matrix:
            ...
            self.src_emb = EmbedMatrix(cfg.src_vocab_size,model_dim, cfg)
            ...
            self.src_emb = Embedding(cfg.model_dim, cfg.src_vocab_size, cfg.embed_dropout)
    ...
    def encode(self, src, src_mask, pair_mask=None, gc_emb=None, energy_emb=None):
    ...
    def forward(self, src, trg, seq_len, gc_content=None, energy=None):
        trg = trg[:, :-1]
        trg_mask = self.make_trg_mask(trg, seq_len)
        trg = trg.masked_fill(trg == -100, 0)
        pair_mask = None
        if self.matrix:
            pair_mask = self.make_pair_mask(src, seq_len)
        src_mask = self.make_src_mask(src, seq_len)
        ...
        enc_out = self.encode(src, src_mask,pair_mask, gc_emb, energy_emb)
        dec_out = self.decode(trg, enc_out, trg_mask, src_mask)
        return self.generator(dec_out)
    
    def generate(self, src, seq_len, gc_content=None,energy=None, max_len=100, greedy=False):
        src_mask = self.make_src_mask(src, seq_len)
        pair_mask = None
        if self.matrix:
            pair_mask = self.make_pair_mask(src, seq_len)
        ...
        energy_emb = self.energy_encoder(energy.unsqueeze(1) / self.energy_scale) if self.energy_cond and energy is not None else None
        enc_out = self.encode(src, src_mask, pair_mask, gc_emb, energy_emb)
        trg = torch.zeros(src.shape[0], 1, dtype=torch.long, device=src.device)
        max_len = seq_len.max()
        for i in range(max_len):
            trg_mask = self.make_trg_mask(trg, torch.ones(trg.shape[0], dtype=torch.long, device=src.device)*(i+1))
            dec_out = self.decode(trg, enc_out, trg_mask, src_mask)
    ...
    def generate_ribo(self, src_seq, src_struct, seq_mask, struct_mask, seq_len, gc_content=None, energy = None, max_len=100, greedy=False, constrained_generation=True):
        src_mask = self.make_src_mask(src_struct, seq_len)
        pair_mask = None
        if self.matrix:
            pair_mask = self.make_pair_mask(src_seq, seq_len)
        seq_emb = self.seq_emb(src_seq) if self.matrix and src_seq is not None else None
        gc_emb = self.gc_encoder(gc_content.unsqueeze(1)) if self.gc_cond and gc_content is not None else None
        energy_emb = self.energy_encoder(energy.unsqueeze(1) / self.energy_scale) if self.energy_cond and energy is not None else None
        enc_out = self.encode(src_struct, src_mask, pair_mask, seq_emb, gc_emb, energy_emb)
