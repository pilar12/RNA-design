...
class EvalRNAformer():
    ...
    def __call__(self, sequence, max_len=100, mean_triual=True):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--file', type=str,default=None)
    ...
    if args.file is not None:
        file = args.file
    pred_files = glob(file+"*_preds.pt")
    for pred_file in pred_files:
        ...
        if os.path.exists(struct_file):
            ...
            for i,sample in enumerate(tqdm.tqdm(dataset)):
                pred_mat = eval_model(sample, max_len=200, mean_triual=True)
